...
#
# modified by I. Burak Ozyurt to add new DataProcessor classes and random seed
# comand line option.
...
flags.DEFINE_integer("seed", 0, "random seed") # IBO
...
class BioREProcessor(DataProcessor):
    """Processor for the bio relation extraction data set."""
    ...
    def get_dev_examples(self, data_dir):
        """There is no dev data. Dev set set is same as test set."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "test.tsv")), "dev")
    ...
    def _create_examples(self, lines, set_type):
        ...
        examples = []
        for (i, line) in enumerate(lines):
            if i == 0 and (set_type == "test" or set_type == "dev") :
                continue
            guid = "%s-%s" % (set_type, i)
            # if len(line) != 3:
            #    for item in line:
...
class BioREChemProtProcessor(DataProcessor):
    """Processor for the ChemProt relation extraction data set."""
...
def main(_):
    ...
    processors = {
        ...
        "biore": BioREProcessor,
        "chemprot": BioREChemProtProcessor,
    }
